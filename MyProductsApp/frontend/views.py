...
from .serialisers import Product, ProductSerializer
...
def RegisterView(request):
    if request.method == "POST":
        ...
        if len(password) < 5:
            ...
            messages(request,"Password must be at least 5 Characters")
...
def LoginView(request):
    if request.method == "POST":
        ...
        if user is not None:
            login(request, user)
            return redirect('http://localhost:5173/home')  # Redirect to the React app
...
def PasswordResetSent(request, reset_id):
    ...
    if PasswordReset.objects.filter(reset_id=reset_id):
        ...
        #redirect to forgot password page if code doesnt exist
...
def search_view(request):
    if request.method == 'GET':
      ...
      if search:
          product = product.filter(Q(name_icontains = search) | Q(description_icontains = search))
